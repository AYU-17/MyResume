/**
 * Seed Script — projects collection
 * Run once:  node data/projects.seed.js
 *
 * This script drops the existing `projects` collection and re-inserts
 * all portfolio projects so the database is always the single source of truth.
 */
...
const projects = [
  ...
  // ──────────────────────────────────────────────────────── 4. CINE WALLET ──
  {
    ...
    tech: ['React', 'Node.js', 'Express.js', 'MongoDB', 'TMDB API'],
    github: 'https://github.com/AYU-17/Cine-Wallet',
    demo: '#',
    overview: {
      description:   'Cine Wallet is a modern movie platform that allows users to discover trending films, search movie details, watch trailers, manage watchlists and explore entertainment content through a sleek cinematic interface.',
      objective:     'To build a production-style movie platform inspired by Netflix, IMDb and TMDB while demonstrating API integration, frontend architecture, responsive design and scalable backend development.',
      problemSolved: 'Movie enthusiasts often jump between multiple websites for ratings, trailers, cast information and recommendations. Cine Wallet centralizes everything into a single modern experience.',
    ...
    features: [
      { icon: '🎬', name: 'Movie Discovery',   description: 'Browse trending, popular, upcoming and top-rated movies with real-time TMDB integration.' },
      { icon: '🔍', name: 'Smart Search',       description: 'Instant search functionality with movie posters, ratings and detailed information.' },
      { icon: '❤️', name: 'Watchlist System',   description: 'Save favorite movies and maintain a personalized entertainment library.' },
      { icon: '▶️', name: 'Trailer Streaming',  description: 'Watch official trailers directly from the platform before choosing a movie.' },
      { icon: '⭐', name: 'Ratings & Reviews',  description: 'View ratings, reviews and audience scores for informed movie selection.' },
      { icon: '📱', name: 'Responsive Design',  description: 'Optimized experience across desktop, tablet and mobile devices.' }
    ],
    techStack: [
      { icon: '⚛️', name: 'React',       type: 'Frontend Framework',  gradient: 'from-cyan-400 to-blue-500' },
    ...
    challenges: [
      {
        challenge: 'Managing large movie datasets while maintaining fast search performance.',
        solution:  'Implemented optimized search logic and efficient API request handling to reduce unnecessary calls.'
      },
      {
        challenge: 'Displaying rich movie information without overwhelming the user interface.',
        solution:  'Used progressive content layouts, collapsible sections and clean card-based design.'
      },
      {
        challenge: 'Keeping movie data fresh while avoiding API limitations.',
        solution:  'Added caching strategies and optimized request frequency for popular endpoints.'
      }
    ],
    timeline: [
  ...
  // ─────────────────────────────────────────── 5. STUDENT MANAGEMENT SYSTEM ──
  {
    ...
    tech: ['Node.js', 'Express.js', 'MongoDB', 'EJS', 'Nodemailer'],
    github: 'https://github.com/AYU-17/student-management-system',
    demo: '#',
    overview: {
      description:   'A complete Student Management System designed to streamline student registration, academic record management, alumni tracking and administrative operations through a centralized dashboard.',
      objective:     'To develop a real-world educational management platform that demonstrates backend architecture, database management, email integration and admin dashboard development.',
      problemSolved: 'Educational institutions often struggle with scattered student records and manual administration processes. This system centralizes student information, alumni tracking and communication into one platform.',
    ...
    features: [
      { icon: '🎓', name: 'Student Registration', description: 'Manage student profiles, personal details, academic records and enrollment information.' },
      { icon: '👨‍💼', name: 'Admin Dashboard',     description: 'Centralized dashboard for managing students, viewing statistics and controlling system operations.' },
      { icon: '🏆', name: 'Alumni Management',    description: 'Move graduated students into the alumni database and maintain their professional information.' },
      { icon: '📧', name: 'Email Integration',    description: 'Automated email notifications using Nodemailer for communication and updates.' },
      { icon: '✏️', name: 'Student Record Updates',description: 'Edit, update and maintain student information through secure admin controls.' },
      { icon: '📱', name: 'Responsive Interface', description: 'User-friendly design optimized for desktop, tablet and mobile devices.' }
    ],
    techStack: [
      { icon: '🟢', name: 'Node.js',      type: 'Backend Runtime',    gradient: 'from-green-400 to-emerald-600' },
      { icon: '🚂', name: 'Express.js',   type: 'Backend Framework',  gradient: 'from-gray-400 to-gray-600' },
      { icon: '🍃', name: 'MongoDB',      type: 'Database',           gradient: 'from-green-400 to-green-700' },
      { icon: '📄', name: 'EJS',          type: 'Template Engine',    gradient: 'from-yellow-400 to-orange-500' },
      { icon: '📧', name: 'Nodemailer',   type: 'Email Service',      gradient: 'from-blue-400 to-indigo-600' },
      { icon: '🎨', name: 'Tailwind CSS', type: 'Frontend Styling',   gradient: 'from-cyan-400 to-teal-500' },
      { icon: '🔐', name: 'JWT',          type: 'Authentication',     gradient: 'from-purple-400 to-pink-500' },
      { icon: '☁️', name: 'Render',       type: 'Deployment Platform',gradient: 'from-indigo-400 to-purple-600' }
    ],
    architecture: [
      { icon: '🖥️', name: 'EJS Frontend',   detail: 'Admin Interface',   gradient: 'from-cyan-400 to-blue-500' },
      { icon: '🔐', name: 'Authentication',  detail: 'Access Control',    gradient: 'from-purple-400 to-pink-500' },
      { icon: '🚂', name: 'Express Server',  detail: 'Business Logic',    gradient: 'from-gray-400 to-gray-600' },
      { icon: '🍃', name: 'MongoDB',         detail: 'Student Database',  gradient: 'from-green-400 to-green-700' }
    ],
    challenges: [
      {
        challenge: 'Managing student lifecycle transitions from active student records to alumni records while preserving data consistency.',
        solution:  'Created dedicated alumni workflows and database structures that allow seamless migration of student data without duplication.'
      },
      {
        challenge: 'Implementing reliable email communication for administrative notifications and updates.',
        solution:  'Integrated Nodemailer with reusable email templates and centralized mailing functions.'
      },
      {
        challenge: 'Maintaining clean CRUD operations across multiple student-related modules.',
        solution:  'Developed modular controllers and reusable database functions to reduce code duplication and improve maintainability.'
      }
    ],
    timeline: [
      { phase: '💡 Requirement Analysis', description: 'Identified core educational administration needs and planned the system architecture.' },
      { phase: '🗄️ Database Design',     description: 'Created MongoDB collections for students, alumni and administrative records.' },
      { phase: '🚂 Backend Development', description: 'Built Express routes, controllers and CRUD functionality for all modules.' },
      { phase: '🎨 Dashboard UI',        description: 'Designed and implemented responsive EJS-based admin dashboard interfaces.' },
      { phase: '📧 Email Integration',   description: 'Integrated Nodemailer for automated communication and notifications.' },
      { phase: '🧪 Testing',             description: 'Tested all CRUD workflows, data validation and email functionality.' },
      { phase: '🚀 Deployment',          description: 'Configured production environment and deployed the application.' }
    ],
    stats: [
      { number: 6,  label: 'Weeks to Build' },
      { number: 6,  label: 'Core Features' },
      { number: 15, label: 'Admin Modules' },
      { number: 3,  label: 'Database Collections' }
    ],
    achievements: [
...
const seed = async () => {
  try {
    ...
    // Drop existing projects so we always have a clean slate
    ...
    inserted.forEach(p => console.log(`   • [${p.order}] ${p.slug} — ${p.title}`));
